...
export const getAdminBusinesses = async () => {
  try {
    ...
    querySnapshot.forEach((doc) => {
      ...
      if (data.businessName) {
        businesses.push({ 
          ...
          businessType: (data.businessType || 'other').toLowerCase().trim()
        });
      }
    });
    ...
    console.warn('Error with specific query, trying fallback:', error);
    try {
      const allUsersSnap = await getDocs(collection(db, 'users'));
      const businesses = [];
      allUsersSnap.forEach(doc => {
        const data = doc.data();
        if (data.role && data.role.toLowerCase().trim() === 'admin' && data.businessName) {
          businesses.push({
            id: doc.id,
            businessName: data.businessName,
            businessType: (data.businessType || 'other').toLowerCase().trim()
...
/**
 * Leave the queue (Customer function)
 */
export const leaveQueue = async (appointmentId) => {
  try {
    await updateDoc(doc(db, 'appointments', appointmentId), {
      status: 'cancelled',
